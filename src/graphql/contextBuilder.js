...
export const createContextBuilder = (path = __dirname, servicesOptions = {}) => (options = {}) => {
    const context = {
        ...
        account: {},
        models: {},
        // Extract caller information from headers or request
        callerRoleId: options.request?.headers?.['x-caller-role-id'],
        internalCall: options.request?.headers?.['x-internal-call'] === 'true',
        metadata: {
            requestId: options.request?.headers?.['x-request-id'] || generateRequestId(),
            traceId: options.request?.headers?.['x-trace-id'],
            timestamp: Date.now(),
            source: options.request?.headers?.['x-source'] || 'external'
        }
    }
